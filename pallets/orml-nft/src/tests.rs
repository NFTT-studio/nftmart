...
fn mint_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(1, NonFungibleTokenModule::tokens(CLASS_ID, TOKEN_ID + 1).unwrap().count);
...
fn mint_should_fail() {
	ExtBuilder::default().build().execute_with(|| {
		...
		Classes::<Runtime>::mutate(CLASS_ID, |class_info| {
			class_info.as_mut().unwrap().total_issuance = <Runtime as Config>::TokenId::max_value() - 10;
		});
		assert_noop!(
